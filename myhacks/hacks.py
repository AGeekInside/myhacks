"""Module of various utility functions and class to support hacks."""
...
class HacksInfo:
    """Utility class that tracks information about hacks in the system."""

    def __init__(self):
        self.hacks_dir = self.get_hacks_dir()

    def get_hacks_dir(self):
        """Determines the directory of the installed hacks."""

        self.hacks_dir = os.path.dirname(inspect.getfile(inspect.currentframe()))
...
def restart_docker():
    ...
    cmds = ["sudo systemctl daemon-reload", "sudo systemctl restart docker"]
...
def check_remote_commits(repo_dir, branch):
    ...
    git_fetch(repo_dir)
    num_changes_cmd = (
        f'bash -c "cd {repo_dir} ; git cherry {branch} origin/{branch} | wc -l"'
    )
...
def get_git_info(repo_dir, fetch=False):
...
def create_repo_table(git_info, fetch, all):
    ...
    repo_table = {"name": [], "num_branches": [], "branch": [], "changes?": []}
...
def convert_bytes(num):
    """
    Function to convert a byte number to KB, MB, etc.
    """
...
def list_files(directory, recursive=True):
    ...
    files_info = [f for f in os.walk(directory)]
